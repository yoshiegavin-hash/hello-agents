...
class PDFLearningAssistant:
    ...
    def load_document(self, pdf_path: str) -> Dict[str, Any]:
        ...
        """
        print(f"[DEBUG] load_document start: {pdf_path}", flush=True)
        if not os.path.exists(pdf_path):
            return {"success": False, "message": f"文件不存在: {pdf_path}"}

        start_time = time.time()
        print(f"[DEBUG] file exists, calling rag_tool... ({time.time()-start_time:.1f}s)", flush=True)
        ...
        try:
            ...
            print(f"[DEBUG] rag_tool done: {process_time:.1f}s", flush=True)
...
def create_gradio_ui():
    ...
    def load_pdf(pdf_file) -> str:
        ...
        print(f"[DEBUG] load_pdf called: pdf_file={type(pdf_file).__name__}", flush=True)
        if assistant_state["assistant"] is None:
            print("[DEBUG] assistant is None", flush=True)
            return "❌ 请先初始化助手"

        if pdf_file is None:
            print("[DEBUG] pdf_file is None", flush=True)
            return "❌ 请上传PDF文件"

        # Gradio 5.x: type="filepath" 直接返回字符串路径
        pdf_path = pdf_file if isinstance(pdf_file, str) else getattr(pdf_file, 'name', str(pdf_file))
        print(f"[DEBUG] 加载PDF: {pdf_path}", flush=True)
    ...
    def chat(message: str, history: List) -> Tuple[str, List]:
        """聊天功能"""
        if assistant_state["assistant"] is None:
            return "", history + [{"role": "assistant", "content": "❌ 请先初始化助手并加载文档"}]
