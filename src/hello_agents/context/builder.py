...
实现Gather-Select-Structure-Compress上下文构建流程:
...
class ContextPacket:
    ...
    ContextPacket是系统中信息的基本单元。每个候选信息都会被封装成一个ContextPacket,包含内容、时间戳、token数量
    和相关性分数等核心属性。这种统一的数据结构简化了后续的选择和排序逻辑
    """
    content: str
...
class ContextConfig:
    ...
    enable_mmr: bool = True # 启用最大边际相关性
...
class ContextBuilder:
    ...
    def _gather(
        ...
        if system_instructions:
            packets.append(ContextPacket(
                ...
                token_count= count_tokens(system_instructions),
                relevance_score=1.0,
                metadata={"type": "instructions", "priority": "high"}
            ))
        
        # 2.从记忆系统检索相关记忆，获取任务状态和关键结论
        if self.memory_tool:
            try:
                # 搜索任务状态相关记忆
    ...
    def _structure(
        ...
        p2_packets = [
            ...
            if p.metadata.get("type") in {"related_memory","knowledge_base", "retrieval", "tool_result", "notes"}
        ]
