...
class MCPTool(Tool):
    ...
    def get_expanded_tools(self) -> List['Tool']:  # type: ignore
        ...
        from tools.buildtin.mcp_wrapper_tool import MCPWrappedTool
...
class A2ATool(Tool):
    ...
    使用示例:
        >>> from tools.buildtin.protocol_tools import A2ATool
        >>> # 连接到A2A Agent
        >>> tool = A2ATool(agent_url ="http://localhost:5000")
        >>> # 连接到A2A Agent(自定义名称和描述)
        >>> tool = A2ATool(
        ...
        ... )
        >>> # 提问
        >>> result = tool.run({"action":"ask", "question":"计算2+2"})
        >>> # 获取信息
