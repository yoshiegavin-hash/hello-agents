...
class MySimpleAgent(Agent):
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        tools_section += "参数可以是 key=value 格式或 JSON 对象格式，如 `path=my_README.md` 或 `{\"path\": \"my_README.md\"}`\n\n"
    ...
    def add_tool(self, tool) -> None:
        ...
        if hasattr(tool, 'auto_expand') and tool.auto_expand:
            ...
            for exp_tool in expanded_tools:
                self.tool_registry.register_tool(exp_tool)
                print(f"🔧 展开工具 '{exp_tool.name}' 已添加")
