...
class WorkingMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 纯内存TTL(分钟)，可通过在MemoryConfig上挂载working_memory_ttl_minutes覆盖
    ...
    def _calculate_time_decay(self, timestamp: datetime) -> float:
        ...
        # 指数衰减(工作记忆衰减更快)
        decay_fator = self.config.decay_factor **(hours_passed / 6) # 每6小时衰减
        return max(0.1, decay_fator)
