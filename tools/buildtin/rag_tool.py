...
class RAGTool(Tool):
    ...
    def __init__(
        ...
        # 双模存储设计：本地缓存+云端向量库
        # 设计意图：这里同时定义了本地文件路径和Qdrant(一个非常高性能的开源向量数据库)的地址
        self.qdrant_api_key = qdrant_api_key or os.getenv("QDRANT_API_KEY")
        self.collection_name = collection_name
        self.rag_namespace = rag_namespace
        """
        设计意图：collection_name（集合）和 rag_namespace（命名空间）。
        为什么好：这是为了防止“数据串门”。比如你既想做“法律法规知识库”，又想做“代码调优知识库”。
        通过设置不同的 rag_namespace，同一个工具就能在底层把这两拨数据隔离开，互不干扰。
        """
        self._pipelines: Dict[str, Dict[str, Any]] = {}
        """
        管线复用与延迟加载:
        设计意图：初始化了一个空字典来准备存放pipelines（处理流水线）
        为什么好：处理 PDF 的流程和处理音频（需要先语音转文字）的流程是完全不同的。这里设计一个字典，
        是为了后面根据文件类型，动态加载不同的处理流水线，提高内存利用率，避免一开始就把所有组件都塞满内存。
        """
